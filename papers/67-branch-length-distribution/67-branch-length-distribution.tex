...
\begin{abstract}
A \emph{Steiner sentence} is a maximal block of consecutive Syracuse-map
applications between two consecutive nodes congruent to~$5 \pmod{8}$ in a
Collatz orbit; its length~$k$ is the number of applications.
...
\begin{remark}
A single application of $\Sm$ together with its output node is called a
\emph{Steiner word} (or \emph{Steiner circuit}); a Steiner sentence is then
a maximal sequence of Steiner words between two consecutive $5 \pmod 8$ nodes.
...
the broader Steiner circuit framework and the regular-expression characterisation
of Collatz paths, but is not required here.
\end{remark}
...
This paper is part of the Speculative Results Series, a programme of incremental
research on the Collatz conjecture. The proof structure was found through
...
The last column gives the probability of reaching class~5 in one step
($P_{15}=1/4$, $P_{35}=1/2$, $P_{75}=0$) — these are the exact entries
