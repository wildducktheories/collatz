...
The transition probability $P[a][b]$ is the fraction of odd integers
$n \equiv a \pmod{8}$ for which $\Sm(n) \equiv b \pmod{8}$,
measured over the natural density.
...
\medskip\noindent
\textbf{Rows 1 and 5 (uniform rows).}
Write $n = 8k + r$ for $r \in \{1, 5\}$. Then $3n + 1 = 24k + (3r+1)$,
so $3n+1 \pmod{8}$ depends only on $r$ modulo~8, not on $k$. For $r=1$:
$3(1)+1=4$, so $3n+1 \equiv 4 \pmod 8$, giving $\vt(3n+1) \geq 2$.
Specifically, $\Sm(n) = (3n+1)/4$ and the residue of $\Sm(n) \pmod 8$
depends on $k$. Since $\gcd(3, 2^j) = 1$, multiplication by~3 is a
bijection on $\mathbb{Z}/2^j\mathbb{Z}$ for every $j \geq 1$. Therefore
as $k$ ranges uniformly over a complete residue system mod~$2^j$, the
value $3n+1 \pmod{2^j}$ is uniform over $2^j\mathbb{Z}$. In particular
$\Sm(n) \pmod 8$ is uniform over $\{1,3,5,7\}$, giving $P[1][b] = 1/4$
for each $b \in \{1,3,5,7\}$. The same argument applies to $r = 5$
(where $3n+1 = 8(3k+2)$ and uniformity of $3k+2 \pmod{2^j}$ follows
from $\gcd(3, 2^j) = 1$).
...
The present paper shows that the $3/4$ decay rate of sentences is a direct
consequence of the same arithmetic:
\begin{itemize}
  \item The $1/4$ probability of immediate termination ($b_0 \equiv 5$) comes from
        the uniform output of class~5 (Paper~66, class~5 is the unique class with
        $c_r \geq 3$, causing its output to be uniform over all four classes).
  \item The $1/2$ termination probability from class~3 comes from the mod-16
        split proved in Theorem~\ref{thm:matrix}: exactly half of class~3 nodes
        map to class~5.
  \item The invariant $d_3 = d_7$ — and hence the geometric decay — follows
        from the structural symmetry between the forbidden transitions of
        classes~3 and~7.
\end{itemize}
