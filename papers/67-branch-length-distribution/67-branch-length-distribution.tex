...
a bijection on $\mathbb{Z}/2^j\mathbb{Z}$, so $3k+2$ hits each residue class
mod $2^{j+3}$ with equal asymptotic density as $k$ ranges over positive integers.
Taking odd parts, $P[5][b] = 1/4$ for all $b \in \{1,3,5,7\}$, as proved in
Theorem~2.1(3) and Proposition~4.1 of Paper~66~\cite{paper66}.

\emph{Note.} Unlike rows~1, 3, and~7 — where the transition targets are
determined by $k \bmod 2$ or $k \bmod 4$ and every finite count is exactly
equal — row~5 is genuinely a natural-density limit.
The finite count $\#\{k < 2^N : \Sm(8k+5) \equiv b \pmod 8\}$ fluctuates
around $2^{N-2}$ without being exactly $2^{N-2}$ at every~$N$:
for example, at $N=6$ and $b \equiv 1$ the count is~17, not~16.
This was verified by the companion Lean file
(\texttt{row5\_count\_not\_exact}).
