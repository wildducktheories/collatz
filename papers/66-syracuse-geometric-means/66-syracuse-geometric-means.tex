...
across all four letter types~\cite{terras,lagarias}, and is the arithmetic
origin of the $3/4$ decay appearing in the Steiner sentence length distribution
...
Steiner words (path segments whose compressed mod-8 form matches
$(7^*3)?(1\mid 5)$) in which the \emph{clause} (the first $k-1$ words,
each terminating at letter~$1$) is followed by a \emph{closing word} (the final word,
terminating at letter~$5$).
...
dynamics within the clause after conditioning on non-termination) has valuation
...
in the clause. It enters only at the sentence boundary (the
uniform output of class~5 that starts each new sentence) and as the termination
condition (class~5 is the absorbing state of the closing word). In both roles, only
row~5 of the transition matrix matters, and that row is uniform by the same
$\gcd(3,2^j)=1$ argument.
...
(See §\ref{sec:background} and Appendix~A of Paper~67~\cite{paper67} for the
full Steiner language hierarchy.)
...
This paper forms part of a programme of incremental research on the Collatz conjecture.
The Steiner circuit framework will be developed fully in future work. Paper~65~\cite{paper65} provides the
