...
contribution.  For cycle problems, the equation~\eqref{eq:cycle} constrains
the basis: given $(g,h)$ and the parity sequence $w$, the value of $a$ is
determined, and $q$ must be chosen to make it an integer.  Typically $q$
is fixed (e.g.\ $q = 1$) and the question becomes whether $(h^e - g^o)
\mid \kpoly_w(g,h)$.
\end{remark}
